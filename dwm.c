...
static unsigned int nextusedtag(void);
static Client *nexttiled(Client *c);
static void pop(Client *);
static unsigned int prevtag(void);
static unsigned int prevusedtag(void);
...
static void statusbarcmd(const Arg *arg);
static void spawn(const Arg *arg);
static pid_t spawn_(char *const *cmd);
...
static void viewnextused(const Arg *arg);
static void viewprev(const Arg *arg);
static void viewprevused(const Arg *arg);
...
spawn_(char *const *cmd)
{
	...
	if ((childpid = fork()) == 0) {
		...
		setsid();
		execvp(cmd[0], cmd);
		fprintf(stderr, "dwm: execvp %s: %s", cmd[0], strerror(errno));
		exit(EXIT_FAILURE);
	}
	return childpid;
}
...
void
viewnextused(const Arg *arg)
{
	view(&(const Arg){.ui = nextusedtag()});
}
